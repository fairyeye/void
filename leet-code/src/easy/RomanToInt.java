...
public class RomanToInt {
    public static void main(String[] args) {
        System.out.println(new RomanToInt().romanToInt2("MCMXCIV"));
    }
     ...
     * 14:54	info
     * 			解答成功:
     * 			执行耗时:8 ms,击败了30.24% 的Java用户
     * 			内存消耗:38.8 MB,击败了80.29% 的Java用户
     * @param s
     * @return
     */
...
//            }
            if ( i < chars.length - 1) {
                if (map.get(chars[i]) < map.get(chars[i + 1])) {
                    sum += map.get(chars[i + 1]) - map.get(chars[i]) - map.get(chars[i + 1]);
                    continue;
                }
            }
            sum += map.get(chars[i]);
